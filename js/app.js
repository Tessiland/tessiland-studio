document.addEventListener('DOMContentLoaded', () => {
    ...
    function applicaDeepLink() {
        ...
        if (filato) {
            const opzione = [...filtroFilatoSelect.options].find(
                o => o.text.toLowerCase() === filato.toLowerCase()
            );
            if (opzione) {
                filtroFilatoSelect.value = opzione.value;
                stato.filtriCatalogo.filatoId = opzione.value;
            }
        }

        const autrice = params.get('autrice');
        if (autrice) {
            const opzione = [...filtroAutriceSelect.options].find(
                o => o.value.toLowerCase() === autrice.toLowerCase()
            );
            if (opzione) {
                filtroAutriceSelect.value = opzione.value;
                stato.filtriCatalogo.autrice = opzione.value;
            }
        }
    }
